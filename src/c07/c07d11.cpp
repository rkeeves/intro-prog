...
const string KEYWORD_LET = "#";
const string KEYWORD_QUIT = "exit";
...
const char KIND_SQRT = 'S';     // Squareroot kind token
const char KIND_POW = 'P';     // Power kind token
  ...
  case ',':
	...
	default:
		if (isalpha(ch) || ch=='#') {
			string s;
			s += ch;
			while(cin.get(ch) && (ch=='_' || ch=='#' || isalpha(ch) || isdigit(ch))) s+=ch;
			cin.putback(ch);
      if (s == KEYWORD_LET) return Token(KIND_LET);
			if (s == KEYWORD_QUIT) return Token(KIND_QUIT);
      if (s == KEYWORD_SQRT) return Token(KIND_SQRT);
      if (s == KEYWORD_POW) return Token(KIND_POW);
...
double primary()
{
  ...
  {
    Token temp = ts.get();
    if(temp.kind != '(') error("Missing opening parentheses after pow");
    double base = expression();
    temp = ts.get();
    if(temp.kind != ',') error("Missing comma in pow arguments");
    int exp_i = narrow_cast<int>(expression());
    temp = ts.get();
    if(temp.kind != ')') error("Missing closing parentheses after pow");
    return pow(base,exp_i);
  }
	case KIND_NUMBER:
		return t.value;
	case KIND_NAME:
